...
template <class T> TSpline1D3<T>::TSpline1D3 ()
{
}



template <class T> TSpline1D3<T>::TSpline1D3 (std::vector<double> const& X, std::vector<T> const& Y)
{
  this->Set(X, Y);
}
...
template <class T> void TSpline1D3<T>::Set (std::vector<double> const& X, std::vector<T> const& Y)
{
  fX.clear();
  fY.clear();
...
template <class T> void TSpline1D3<T>::Set (double const* X, T const* Y, int const N)
{
  if (N <= 0) {
    throw;
  }
...
template <class T> T TSpline1D3<T>::GetValue (double const x) const
{
...
template <class T> void TSpline1D3<T>::Derivatives ()
{
  // Number in vectors
  ...
  double const dydx0 = (fY[1] - fY[0]) / (fX[1] - fX[0]);
  if (dydx0 > 0.99e30) {
    fYPP[0] = 0;
    u[0] = 0;
  } else {
  ...
  double qn;
  double un;
  double const dydxN = (fY[N-1] - fY[N-2]) / (fX[N-1] - fX[N-2]);
  if (dydxN > 0.99e30) {
    qn = 0;
    un = 0;
  } else {
  ...
  fYPP[N-1] = (un - qn * u[N-2]) / (qn * fYPP[N-2] + 1.);
...
TSpline1D3_1d::TSpline1D3_1d () : TSpline1D3<double>()
{
}
...
TSpline1D3_1d::~TSpline1D3_1d ()
{
}



void TSpline1D3_1d::ReadFile (std::string const& InFileName)
{
  std::vector<double> X;
  std::vector<double> Y;
