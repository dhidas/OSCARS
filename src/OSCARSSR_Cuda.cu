                                       ...
                                       double *flux)
{
  ...
  // Is this thread a valid surfface point?
                                                        ...
                                                        int const MaxLevel)
{
  ...
  *h_ns     = (int) Surface.GetNPoints();
  ...
  for (size_t i = 0; i < GPUsToUse.size(); ++i) {
    cudaHostAlloc((void**) &(h_result[i]), NFlux * sizeof(double),  cudaHostAllocWriteCombined | cudaHostAllocMapped);
  }
  ...
  *h_prec = Precision;
  ...
  for (size_t i = 0; i < GPUsToUse.size(); ++i) {
    ...
    cudaMalloc((void **) &d_ml[i],                 sizeof(int));
    cudaMalloc((void **) &d_prec[i],               sizeof(double));
    cudaMalloc((void **) &d_result[i],       NFlux * sizeof(double));
  ...
  *h_tstop  = T.GetXStop();
  ...
  for (int ip = 0; ip < NParticlesReally; ++ip) {
    ...
    for (size_t ig = 0; ig < GPUsToUse.size(); ++ig) {
      int const d  = GPUsToUse[ig];
      cudaSetDevice(d);
      cudaMemcpyAsync(h_result[ig],  d_result[ig],  NFlux * sizeof(double), cudaMemcpyDeviceToHost);
