                                       ...
                                       double *result)
{
  ...
  for (int ilevel = *nl; (ilevel <= *ml) && !_all_done; ++ilevel) {
    ...
    if (in_surface && !done) {
      ...
      // If below desired precision set as done
      if ( (ilevel > 8 && result_precision < *prec && MaxDPhase < PI) || (ilevel > 8 && MaxDPhase < PI && this_result == last_result) ) {
        done = true;
        result_level = ilevel;
      }
...
extern "C" void OSCARSSR_Cuda_CalculateFluxGPU_TEST (OSCARSSR& OSR,
                                                ...
                                                int    const ReturnQuantity)
{
  ...
  int const NLevelsPreComputed = 15;
  ...
  int *h_ns;
  int *h_nt;
  int *h_nl;  // Number of pre-computed trajectory levels
  ...
  cudaHostAlloc((void**) &h_nl,     sizeof(int),    cudaHostAllocWriteCombined | cudaHostAllocMapped);
  ...
  *h_nl     = NLevelsPreComputed;
  ...
  cudaHostAlloc((void**) &h_tbx,    NTPreComputed * sizeof(double),  cudaHostAllocWriteCombined | cudaHostAllocMapped);
  cudaHostAlloc((void**) &h_tby,    NTPreComputed * sizeof(double),  cudaHostAllocWriteCombined | cudaHostAllocMapped);
  cudaHostAlloc((void**) &h_tbz,    NTPreComputed * sizeof(double),  cudaHostAllocWriteCombined | cudaHostAllocMapped);
  cudaHostAlloc((void**) &h_tax,    NTPreComputed * sizeof(double),  cudaHostAllocWriteCombined | cudaHostAllocMapped);
  cudaHostAlloc((void**) &h_tay,    NTPreComputed * sizeof(double),  cudaHostAllocWriteCombined | cudaHostAllocMapped);
  cudaHostAlloc((void**) &h_taz,    NTPreComputed * sizeof(double),  cudaHostAllocWriteCombined | cudaHostAllocMapped);
  ...
  *h_rt = ReturnQuantity;
  ...
  cudaHostAlloc((void **) &d_nl,     NGPUsToUse * sizeof(int*),     cudaHostAllocWriteCombined | cudaHostAllocMapped);
  ...
  for (size_t i = 0; i < GPUsToUse.size(); ++i) {
    ...
    cudaMalloc((void **) &d_nl[i],                 sizeof(int));
  ...
  *h_omega = TOSCARSSR::EvToAngularFrequency(Energy_eV);
  ...
  for (size_t i = 0; i < GPUsToUse.size() - 1; ++i) {
    ...
    cudaMemcpyPeerAsync( d_nl[i+1],     d1, d_nl[i],     d, sizeof(int));
  ...
  *h_tstop  = T.GetXStop();
  ...
  for (int ig = 0; ig < NGPUsToUse; ++ig) {
    ...
    std::cout << "create cuda event: " << event_resultcopy[ig] << std::endl;
  }
  ...
  for (int ip = 0; ip < NParticlesReally; ++ip) {
    ...
    for (size_t ig = 0; ig < GPUsToUse.size(); ++ig) {
      ...
      std::cout << "event sync of: " << event_resultcopy[ig] << std::endl;
    ...
    for (size_t ig = 0; ig < GPUsToUse.size(); ++ig) {
      ...
      cudaEventRecord(event_resultcopy[ig]);
      std::cout << "event recorded: " << event_resultcopy[ig] << std::endl;
    }
  ...
  for (size_t ig = 0; ig < GPUsToUse.size(); ++ig) {
    cudaEventSynchronize(event_resultcopy[ig]);
    std::cout << "event sync: " << event_resultcopy[ig] << std::endl;
  }
  ...
  std::cout << "begin memfree" << std::endl;
  // Free host memory
  cudaFreeHost(h_nl);
  cudaFreeHost(h_tx);
  cudaFreeHost(h_ty);
  ...
  cudaFreeHost(h_nt);
  cudaFreeHost(h_t);
  cudaFreeHost(h_tstart);
  cudaFreeHost(h_tstop);
  cudaFreeHost(h_ns);
  cudaFreeHost(h_pol);
  cudaFreeHost(h_x);
  cudaFreeHost(h_y);
  ...
  std::cout << "return after memfree" << std::endl;
                                                ...
                                                int    const ReturnQuantity)
{
  ...
  *h_tstop  = T.GetXStop();
  ...
  for (size_t ig = 0; ig < GPUsToUse.size(); ++ig) {
    cudaEventSynchronize(event_resultcopy[ig]);
    std::cout << "got event ig " << ig << std::endl;
  }
                                               ...
                                               double *result)
{
  ...
  for (int ilevel = 0; (ilevel <= *ml) && !_all_done; ++ilevel) {
    ...
    if (in_surface && !done) {
      ...
      // If below desired precision set as done
      if ( (ilevel > 8 && result_precision < *prec && BetaDiffMax < 2. / *gamma) || ilevel > 8 && BetaDiffMax < 2. / *gamma && this_result == last_result) {
        done = true;
        result_level = ilevel;
